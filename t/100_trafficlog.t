...

{
    ...
    $test->()->($req);
    is @log, 2;
    like $log[0], qr{^\[\d{2}/\S+/\d{4}:\d{2}:\d{2}:\d{2} \S+\] \[\d+\] \[127.0.0.1 -> example.com:80\] \[Request\] POST / HTTP/1.1\|Host: example.com\|Content-Length: 10\|Content-Type: text/plain\|\|$};
    like $log[1], qr{^\[\d{2}/\S+/\d{4}:\d{2}:\d{2}:\d{2} \S+\] \[\d+\] \[127.0.0.1 -> example.com:80\] \[Response\] HTTP/1.0 200 OK\|Content-Type: text/plain\|\|$};
    ...
    is @log, 1;
    like $log[0], qr{^\[\d+\] \[127.0.0.1 -> example.com:80\] \[Response\] HTTP/1.0 200 OK\|Content-Type: text/plain\|\|$};

    @log = ();
    $test->(with_date => 0, with_body => 1)->($req);
    is @log, 2;
    ...
    @log = ();
    $test->(with_date => 0, with_body => 1, eol => '!')->($req);
    is @log, 2;
    like $log[0], qr{^\[\d+\] \[127.0.0.1 -> example.com:80\] \[Request\] POST / HTTP/1.1!Host: example.com!Content-Length: 10!Content-Type: text/plain!!TEST!TEST!$};
    like $log[1], qr{^\[\d+\] \[127.0.0.1 -> example.com:80\] \[Response\] HTTP/1.0 200 OK!Content-Type: text/plain!!OK!OK!$};

    @log = ();
    $test->(with_date => 0, with_body => 1, eol => '!', body_eol => ',')->($req);
    is @log, 2;
    like $log[0], qr{^\[\d+\] \[127.0.0.1 -> example.com:80\] \[Request\] POST / HTTP/1.1!Host: example.com!Content-Length: 10!Content-Type: text/plain!!TEST,TEST,$};
    like $log[1], qr{^\[\d+\] \[127.0.0.1 -> example.com:80\] \[Response\] HTTP/1.0 200 OK!Content-Type: text/plain!!OK,OK,$};
}
